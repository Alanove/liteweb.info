...
(function ($) {
	...
	$.fn.lwSlideShow = function (options) {
		...
		var currentLoadedImage = 1;
		var current = 1;

		var $el = this;
		var animateInterval = null;
		var images = $el.children().length > 0 ? [$($el.children()[0])] : [];
		...
		function animate(a, b, stopAfter, next) {
			if (opts.max <= 1)
				animateFlag = false;
			...
			if (animateFlag) {
				...
				$el.find('.bullets li').removeClass('selected');
				$el.find("#bullet-" + current).addClass('selected');
			}
		}
		...
		function loadNextImage() {
			...
			var img = $("<img />");
			img.attr("src", opts.folder + "/" + currentLoadedImage + "." + opts.extension);
			...
			$el.find("#bullet-1").addClass('selected');
		...
		function init() {
			...
			animateInterval = setTimeout(animate, opts.wait);
		}
		...
		if (opts.parallax === true) {
			function CheckScroll() {
				...
				if (scrollTop > pos.top) {
					...
					img.css({ position: "absolute", top: (scrollTop - pos.top) / opts.parallaxDensity });
					slogansContainer.css({ top: (scrollTop - pos.top) / (opts.slogansParallaxDensity) });
	...
	this.each(function () {
		...
		if (!options.container) {
			...
			div.css({
				...
				"min-width": el.css("min-width"),
				"min-height": el.css("min-height"),
		...
		for (var i = 0; i < stickyMap.length; i++) {
			...
			if (scrollTop > obj.pos.top + obj.options.offset.top) {
				...
				if (obj.isSticky === true) {
					obj.el.css({ position: obj.position, top: obj.top });
