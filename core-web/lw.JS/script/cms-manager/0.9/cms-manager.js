...
//this version is used only in the digital signage project.
...
lw.manager = {
	...
	init: function (obj) {
		...
		_.header.find(".logo").animate({ height: 20 }, 300);
		_.header.find(".logo img").animate({ width: 119 }, 65);
		//_.header.find(".logo").animate({ bottom: -20 }, 300);
		_.header.find(".logo img").attr("src", "images/logo-small.png");
		$("body").addClass("cms-inside");
		...
		$.history.init(function (hash) {
			...
			if (!_.query["section"])
				_.query["section"] = "ZONESANDSCREENS";
	...
	createTab: function (tab, _section, sub) {
		...
		if (!isOk(_section["tabsindex"][tabName])) {
			if (!isOk(path)) {
	...
	onResize: function () {
		var _ = lw.manager;
		_.main.css({ height: GetViewHeight() - 110, width: GetViewWidth() - 20 });
		_.menu.css({ height: GetViewHeight() - 110 });
		_.activeArea.css({ height: GetViewHeight() - 110, width: GetViewWidth() - _.menu.width() - 20 });
