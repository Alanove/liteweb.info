...
namespace CKEditor.NET
{
	...
	public class CKEditorControl : HtmlTextArea, IPostBackDataHandler
	{
		#region LW Page
		CustomPage _page;
		/// <summary>
		...
		public string BasePath
		{
			get
			{
				...
				if (obj == null)
				{
					obj = WebUtils.GetFromWebConfig("CKeditor:BasePath");
		...
		public short TabIndex { get { return (short)config.tabIndex; } set { config.tabIndex = value; } }
...
even if the aligment style is not explicitly applied to the current paragraph in the editor.")]
		...
		{
			MyPage.RegisterLoadScript("CKEDITOR_BASEPATH", string.Format("window.CKEDITOR_BASEPATH = '{0}/';\n", (this.CKEditorJSFile.StartsWith("~") ? this.ResolveUrl(this.BasePath) : this.BasePath)), true);
			MyPage.RegisterScriptFile("ckeditor", (this.CKEditorJSFile.StartsWith("~") ? this.ResolveUrl(this.CKEditorJSFile) : this.CKEditorJSFile) + timestamp);
			...
			MyPage.RegisterLoadScript("CKEditorForNet", scriptInit, true);
			MyPage.RegisterLoadScript(this.ClientID + @"_addControl", string.Format(@"CKEditor_Controls.push('{0}');
", this.ClientID), true);
...
", this.ClientID, prepareJSON(), doPostBackScript);
			...
			MyPage.RegisterLoadScript("aspintegratorInitial_" + this.ClientID, script);
