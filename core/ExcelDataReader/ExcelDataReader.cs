...
namespace lw.ExcelDataReader
{
    ...
    public class ExcelDataReader
    {
		...
		ExcelDataReader _reader;

		/// <summary>
		/// Returns the excel workbook
		/// Sheets as <see cref="DataTable"/>
		/// ex: excel.WorkbookData.Tables[0] will return the first sheet
		/// </summary>
		/// <param name="file">the file name</param>
		public ExcelDataReader(string file)
		{
			m_file = new FileStream(file, FileMode.Open, FileAccess.Read, FileShare.Read);
			Init();
			m_file.Close();
		}
        ...
        private bool ReadWorksheet(XlsWorksheet sheet)
        {
            ...
            if (idx != null)
            {
                ...
                {
					dims.IsV8 = isV8;
                    maxCol = isV8? dims.LastColumn*2 : dims.LastColumn;
